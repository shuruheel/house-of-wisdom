...
def load_embeddings(filename='concept_embeddings.json'):
    with open(filename, 'r') as f:
        return json.load(f)

def get_processed_concepts(tx):
    query = """
    MATCH (e:Concept) 
    WHERE e.embedding IS NOT NULL 
    RETURN apoc.util.md5([e.name]) AS id
    """
    return [record["id"] for record in tx.run(query)]

def update_concept_embeddings_batch(tx, embeddings_batch):
    query = """
    UNWIND $embeddings AS embedding
    MATCH (e:Concept)
    WHERE apoc.util.md5([e.name]) = embedding.id
...
def main():
    ...
    with neo4j_driver.session() as session:
        processed_concepts = session.execute_read(get_processed_concepts)
    logging.info(f"Found {len(processed_concepts)} already processed concepts")
    
    embeddings_to_process = {k: v for k, v in embeddings.items() if k not in processed_concepts}
    ...
    with neo4j_driver.session() as session:
        for i in tqdm(range(0, len(embeddings_to_process), batch_size), desc="Processing batches"):
            ...
            updated_count = session.execute_write(update_concept_embeddings_batch, embeddings_batch)
            total_updated += updated_count
            
            if i % (batch_size * 10) == 0 and i > 0:
                logging.info(f"Processed {i} embeddings. Total updated so far: {total_updated}")

    logging.info(f"Total concept embeddings updated in Neo4j: {total_updated}")
